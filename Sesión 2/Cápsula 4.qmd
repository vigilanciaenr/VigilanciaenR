...
Lo que tenemos que hacer para importar esta hoja de cálculo a R como un marco de datos es pasar la ruta al archivo Excel al argumento de la función `read_excel()`.
...
Si se va a compartir el código de R, es importante poner la ruta relativa y no la ruta del archivo completa y que la ruta relativa esté en la carpeta del proyecto de R.
...
Si queremos **importar solo una hoja** de nuestro data set tendríamos que escribir la función de esta manera. Este código es solo un ejemplo, no correrá ya que no es adecuado el nombre de ruta, en ese caso tendrían que colocar la ruta de sus respectivos dataset.
...
# Lista todos los nombres de las hojas en el archivo
...
Es importante para este paquete crear antes un proyecto de R donde estarán contenidos nuestros datos.
...
Para ello usamos la función `read_csv()` del paquete `readr` para importar un conjunto de datos con valores delimitados por comas. Esos valores se importarán como un marco de datos y asignamos ese marco de datos al objeto R llamado `data_csv`.
...
Para cargar una tabla que tenemos en Google Drive debemos importarla desde el paquete `googlesheet4`. En el siguiente [enlace](https://googlesheets4.tidyverse.org/) puede consultarse más información sobre el paquete. 

Primero instalamos el paquete necesario. Además necesitaremos utilizar para la autenticación el paquete `httpuv`, ya que es necesario que la cuenta que utlicemos de Google se concete con R para realizar la transferencia de datos.
...
El `"gs4_auth()"` abrirá una ventana emergente que nos pedirá permiso para conectarse a nuestra cuenta Google. Una vez aceptado esto, se procede a importar los datos del drive.
...
Para abrir la herramienta de importación de datos de RStudio, haga click en el **Import Dataset** (menú desplegable cerca de la parte superior del panel del entorno). Allí se elige el tipo de archivo que se quiere importar.
